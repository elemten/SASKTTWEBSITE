...
const SPEDBookingForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Edge Function does EVERYTHING now (DB + Google Calendar)
      const { data, error } = await supabase.functions.invoke("google-calendar-function", {
        body: { action: "bookSlot", booking: bookingData },
      });

      // If Supabase reported non-2xx
      const status = (error as any)?.context?.status;

      // Edge returns structured conflicts
      if (status === 409 || data?.code === "DOUBLE_BOOKED") {
        if (formData.booking_date) fetchAvailableTimeSlots(formData.booking_date);
        throw new Error(data?.error || "This slot was just booked by another teacher. Please pick another time.");
      }

      if (error) {
        throw new Error(data?.error || data?.message || `Booking failed: ${error.message}`);
      }
      ...
      // optional: you can display link if you want
      // console.log("Event:", data.eventId, data.eventLink);
  ...
  return (
    <div className="max-w-4xl mx-auto" id="booking-form">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-green-700">
              ...
              <br />
              323 4th Avenue South, Saskatoon, SK
            </p>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">School Type</CardTitle>
          </CardHeader>
          <CardContent>
            <div>
                ...
                {isCalendarOpen && (
                  <>
                    ...
                    <div
                      ...
                      style={{ top: "calc(100% + 4px)", left: 0, right: 0, maxWidth: "320px" }}
                    >
                      <Calendar
                        mode="single"
                    ...
                    {availableTimeSlots.map((slot, index) => {
                      ...
                      return (
                        <button
                          key={`${slot.time}-${index}`}
                          type="button"
                          className={`w-full text-left px-4 py-2 border rounded-md ${!slot.available ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:bg-gray-50"
