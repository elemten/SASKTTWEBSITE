...
const PROGRAM_CODE = "4310 – SPED Program";
...
// Invoice number: YYYY-MM-SYSCODE-INITIALS (e.g., 2025-12-CATH-HC)
...
serve(async (req) => {
  ...
  try {
    ...
    if (profileError || !profile) {
      ...
      return new Response(JSON.stringify({ error: "User profile not found. Please contact an administrator." }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!["admin", "finance_admin"].includes(profile.role)) {
      return new Response(JSON.stringify({ error: "Forbidden - admin or finance_admin role required" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!monthStart || !schoolSystem || !schoolName) {
      return new Response(JSON.stringify({ error: "Missing required fields: monthStart, schoolSystem, schoolName" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // Create invoice number (MUST exist before any use)
    ...
    page.drawText("info@ttsask.ca · ttsask.ca", {
      x: 50,
      y,
      size: 10,
      font: helveticaFont,
    ...
    y -= 14;
    page.drawText("510 Cynthia St, Saskatoon, SK S7L 7K7", {
      x: 50,
      y,
      size: 10,
      font: helveticaFont,
    ...
    page.drawText(
      `Invoice Date: ${today.toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`,
      {
        x: 50,
        y,
        size: 10,
    ...
    page.drawText(`Program / COA Code: 4310 – SPED Program`, {
      x: 50,
      y,
      size: 10,
      font: helveticaBold,
    ...
    for (const line of paymentText.split("\n")) {
      if (y < 80) {
        currentPage = pdfDoc.addPage([612, 792]);
        y = currentPage.getSize().height - 60;
      }
